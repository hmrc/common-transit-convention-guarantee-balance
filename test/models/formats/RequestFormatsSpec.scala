...
class RequestFormatsSpec extends AnyFlatSpec with Matchers {
  ...
  "BalanceRequest.balanceRequestFormat" should "parse valid JSON as BalanceRequest" in {
  ...
  it should "round trip balance request" in {
    Json.toJson(validResult).as[BalanceRequest] shouldBe validResult
  }
}
